...
def plot_page_counts(data, n):
    plt.style.use("default")
    counts = data.groupby('page').size().sort_values(ascending=True)
    counts_df = counts.to_frame(name='counts').reset_index()
    counts_df[-n:].plot.barh(x='page')


def plot_related_pages(combination_counts_df, page_title, n=20):
